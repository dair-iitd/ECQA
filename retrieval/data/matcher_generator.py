...
def Dump_E2_data(total_data, filename):
	final_data = {'q_text':  total_data['q_text'], 'option': total_data['option'],
	 				'correct': total_data['correct'], 'property': total_data['properties']}
...
def generate_data(df):
	...
	for ind in df.index:
		...
		for i in range(1,6):
			if type(df['q_op'+str(i)][ind]) == type('string'):
				...
				skipFlag = True
				# skip this row as some issue in options format
		if skipFlag:
			continue
		...
		for i in range(1,6):
			...
			if curr_option==df['q_ans'][ind].strip().lower():
				...
				if found:
					...
					correct.pop()
					continue
				# bipartite matching for negative options
			...
			if not correct[-1]:
				neg_degree.append(len(linked_props))
			if len(linked_props)==0:
				q_no.pop()
				q_text.pop()
				option.pop()
				correctness = correct.pop()
				wasted += 1
				if not correctness:
					blank_negs += 1
			else:
	...
	all_properties = [prop.lower().translate({ord(c): '' for c in "!@#$%^&*()[]{};:,./<>?\|`~-=_+"}) for prop in all_properties if (prop.strip())]	# removes empty lines
	total = len(q_no)
	print('Total = ', total, ' Wasted Ops in E2 = ', wasted)
	print('Avg Negative option degree = ', sum(neg_degree)/len(neg_degree) , '# Zero Degree Neg Ops = ', blank_negs)
	e2_data = {'q_no':  q_no, 'q_text': q_text, 'option': option,
